...
def create_inout_sequences(input_data, tw):
    ...
    return torch.FloatTensor(inout_seq) # 将list ,numpy转化为tensor
...
def get_data():
    ...
    # a = series['Temp']
    # amplitude = a.values
    series = read_csv('us.csv')#, header=0, index_col=0, parse_dates=True, squeeze=True)
    a = series['cases']
    amplitude = a.values
    # looks like normalizing input values curtial for the model(将数据缩放至给定的最小值'-1'与最大值'1'之间)
...
def predict_future(eval_model, data_source,steps):
    ...
    # -1在这里的意思是让电脑帮我们计算。view()返回的tensor和传入的tensor共享内存
